...
/**
 * 2.3. Сборка элементов. Сборка в Map
 * Для того чтобы собрать данные в мапу, мы можем использовать
 * методы collect() и Collectors.toMap(). Сборка идет в HashMap
 *
 * Например,
 * Map<Integer, Integer> result = Set.of(1, 2).stream().collect(Collectors.toMap(k -> k, v -> v));
 * В качестве первого аргумента мы указываем, как мы будет строить
 * ключ из элемента стрима, а в качестве второго как будем строить значение.
