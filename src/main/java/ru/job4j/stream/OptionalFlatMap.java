 ...
 * Вам нужно получить только счета пользователя, которые хранятся его в учетной записи
 * (у пользователя одна учетная запись, а в ней есть список счетов).
 * Если вам нужно как-то обработать счета пользователя, то можно сделать преобразование из одного Optional
 * в другой Optional методом flatMap, который есть в Optional. Данный метод производит преобразование только
 * если существует значение в Optional, иначе возвращает пустой Optional. В нашем случае можно поступить так
...
public class OptionalFlatMap {
    public static Optional<Integer> flatMap(List<String> strings) {
        return strings.stream()
                ...
                .flatMap(u -> Optional.of(u.length()));
